...
public class HandDeserialiser
{
    ...
    {
        Debug.Log(json);
        handPoints = JsonUtility.FromJson<HandPoints>(json);
    }
}
...
public class websocketlistener : MonoBehaviour
{
    ...
    HandDeserialiser handDeserialiser = new HandDeserialiser();
    ...
    async void Start()
    {
        handDeserialiser.PointsFromJson(testJson);
    ...
    async void ReceiveData()
    {
        ...
        if (await semaphore.WaitAsync(0))
        {
            try
            {
                ...
                handDeserialiser.PointsFromJson(JsonString.ToString());
            }
            finally
            {
                semaphore.Release();
